...
public class Container {
    ...
    private static class ConnectPostData {
        ...
        public ConnectPostData() {
            this.totalAmount = group1.size() * amount1 + group2.size() * amount2;
        }
    }
    ...
    private boolean postConnect(ConnectPostData postData) {
        return areGroupMembersCorrect(postData)
                && isGroupTotalAmountCorrect(postData)
                && isGroupBalanced()
                && isGroupConsistent();
    }

    private boolean isGroupTotalAmountCorrect(ConnectPostData postData) {
        return almostEqual(groupAmount(), postData.totalAmount);
    }
